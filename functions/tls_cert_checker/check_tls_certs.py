"""
check_tls_certs by @fschulze - https://github.com/fschulze/check-tls-certs
...
def get_domain_certs(domains):
    ...
    (done, pending) = loop.run_until_complete(
        asyncio.wait([loop.run_in_executor(None, get_cert_from_domain, x) for x in itertools.chain(*domains)])
    )
    # no need to close the loop when running as a function, as we will use for consecutive invocations
...
def validate_certificate_chain(cert_chain, msgs):
    ...
    for index, cert in reversed(list(enumerate(cert_chain))):
        ...
        try:
            ...
            msgs.append(("error", "Validation error '%s'." % e))
        if index > 0:
...
def check(domainnames_certs, utcnow, expiry_warn=default_expiry_warn):
    ...
    for domain, cert_chain in domainnames_certs:
        ...
        if not any(isinstance(cert, OpenSSL.crypto.X509) for cert in cert_chain):
            msgs.append(("error", "Couldn't fetch certificate for %s.\n%s" % (domain, cert_chain)))
            continue
        ...
        if expires < utcnow:
            msgs.append(("error", "The certificate has expired on %s." % expires))
        elif expires < (utcnow + datetime.timedelta(days=expiry_warn)):
            msgs.append(("warning", "The certificate expires on %s (%s)." % (expires, expires - utcnow)))
        else:
            # rounded delta
            delta = ((expires - utcnow) // 60 // 10 ** 6) * 60 * 10 ** 6
            msgs.append(("info", "Valid until %s (%s)." % (expires, delta)))
        alt_names = set()
        for index in range(cert.get_extension_count()):
            ext = cert.get_extension(index)
            if ext.get_short_name() != b"subjectAltName":
                continue
            alt_names.update(x.strip().replace("DNS:", "") for x in str(ext).split(","))
        alt_names.add(cert.get_subject().commonName)
        unmatched = domainnames.difference(alt_names)
        if unmatched:
            msgs.append(("info", "Alternate names in certificate: %s" % ", ".join(sorted(alt_names, key=domain_key))))
            if len(domainnames) == 1:
                ...
                msgs.append(("warning", "Unmatched alternate names %s." % ", ".join(sorted(unmatched, key=domain_key))))
        elif domainnames == alt_names:
            msgs.append(("info", "Alternate names match specified domains."))
        else:
            unmatched = alt_names.difference(domainnames)
            msgs.append(
                ("warning", "More alternate names than specified %s." % ", ".join(sorted(unmatched, key=domain_key)))
            )
...
def main(file, domain, expiry_warn, verbose):
    ...
    """
    domains = list(itertools.chain(domain_definitions_from_filename(file), domain_definitions_from_cli(domain)))
