...
class NodeJSDenpendencyTester(AbstractDependencyTester):
    ...
    def test(self, artifact_location: str) -> (str, list):
        # tests node dependency files, returns a list of vulnerabilities
        # returns an empty list if no vulnerabilities are found
        error = None
        vulns = []

        # check if artifact contains a package.json file
        package_json = (
            None
            if not glob.glob(pathname=f"{artifact_location}/**/package.json", recursive=True)
            else glob.glob(pathname=f"{artifact_location}/**/package.json", recursive=True)[0]
        )

        if package_json:
            with open(package_json, "r") as pj:
                try:
                    api_response = self.snyk_org.test_packagejson(pj)
                    if api_response.issues.vulnerabilities:
                        vulns = api_response.issues.vulnerabilities
                except Exception:
                    error = f"{ERROR_PREFIX} There was an error getting the vulnerabilities for the artifact."
        else:
            error = f"{ERROR_PREFIX} No package.json file could be found, please include it in the root of the function archive."
