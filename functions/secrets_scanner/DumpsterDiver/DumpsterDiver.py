...
if __name__ == "__main__":
    ...
    basic.add_argument(
        "-p", dest="local_path", required=True, help="path to the folder containing files to be analyzed"
    )
    basic.add_argument(
        "-r",
        "--remove",
        action="store_true",
        help="when this flag is set, then files which don't contain" + " any secret will be removed.",
    )
    basic.add_argument(
        "-a",
        "--advance",
    ...
    basic.add_argument(
        ...
        choices=range(4),
        metavar="[0,3]",
        help="R|0 - searches for short (20-40 bytes long) keys, \n"
    ...
    configuration.add_argument(
        "--min-key", dest="min_key", type=int, help="specifies the minimum key length to be analyzed (default is 20)."
    )
    configuration.add_argument(
        "--max-key",
        dest="max_key",
        action="store",
        type=int,
        help="specifies the maximum key length to be analyzed (default is 80).",
    )
    configuration.add_argument(
        "--entropy", action="store", type=float, help="specifies the edge of high entropy (default is 4.3)."
    )
    configuration.add_argument(
        "--min-pass",
        dest="min_pass",
        action="store",
        type=int,
        help="specifies the minimum password length to be analyzed"
    ...
    configuration.add_argument(
        ...
        action="store",
        type=int,
        help="specifies the maximum password length to be analyzed"
        + " (default is 12). Requires adding '-s' flag to the "
        + "syntax.",
    )
    configuration.add_argument(
        "--pass-complex",
        dest="password_complexity",
        type=int,
        choices=range(1, 10),
        help="specifies the edge of password "
    ...
    configuration.add_argument(
        ...
        action="store",
        nargs="+",
        help="specifies the grep words to look for. Multiple words should be "
    ...
    try:
        ...
        if arguments.min_key:
            core.MIN_KEY_LENGTH = arguments.min_key
        if arguments.max_key:
            core.MAX_KEY_LENGTH = arguments.max_key
        if arguments.entropy:
            core.HIGH_ENTROPY_EDGE = arguments.entropy
        if arguments.min_pass:
            core.MIN_PASS_LENGTH = arguments.min_pass
        if arguments.max_pass:
            core.MAX_PASS_LENGTH = arguments.max_pass
        ...
        if arguments.exclude_files:
            core.EXCLUDED_FILES = arguments.exclude_files
        if arguments.bad_expressions:
            core.BAD_EXPRESSIONS = arguments.bad_expressions
